...
@Component({
  ...
  styleUrls: ['./task-by-user.component.css']
})
export class TaskByUserComponent {
  ...
  onSubmit() {
    ...
    this.taskService.getByUserSurveillanceTask(this.selectedUser)
      .pipe(
        tap((response) => {
    ...
    this.taskService.getByUserPickupAndDelivery(this.selectedUser)
      .pipe(
        tap((response) => {
